            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <main style={{ padding: '2rem' }}>
                    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        <div>
                            <h2 style={{ fontSize: '1.5rem', fontWeight: '800', color: '#111827', letterSpacing: '-0.025em' }}>My Orders</h2>
                            <p style={{ color: '#6B7280', fontSize: '0.875rem' }}>Ready for a refill, {user.name ? user.name.split(' ')[0] : 'Customer'}?</p>
                        </div>
                        <button 
                    ...
                    {loading ? (
                        ...
                        <div style={{ background: 'white', borderRadius: '1rem', border: '1px solid #E5E7EB', padding: '4rem 2rem', textAlign: 'center' }}>
                            ...
                            <h3 style={{ fontSize: '1.25rem', fontWeight: '700', color: '#111827', marginBottom: '0.5rem' }}>No active orders</h3>
                            <p style={{ color: '#6B7280', marginBottom: '2rem' }}>You haven't placed any delivery orders yet.</p>
                            <button 
                        ...
                        <div style={{ background: 'white', borderRadius: '1.25rem', border: '1px solid #E5E7EB', overflow: 'hidden' }}>
                            <h4 style={{ fontSize: '1.1rem', fontWeight: '700', padding: '1.5rem', borderBottom: '1px solid #F3F4F6', color: '#111827' }}>Order History</h4>
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                {orders.map((order, i) => (
                                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.5rem', borderBottom: i === orders.length - 1 ? 'none' : '1px solid #F3F4F6' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                                            <div style={{ padding: '0.75rem', background: '#EEF2FF', borderRadius: '1rem' }}><Truck size={22} color="#4F46E5" /></div>
                                            <div>
