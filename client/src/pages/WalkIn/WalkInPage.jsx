...
const WalkInPage = () => {
    ...
    return (
        <div className="dashboard-container" style={{ background: '#F9FAFB', display: 'flex', minHeight: '100vh' }}>
            <Sidebar role="staff" />
                ...
                <main style={{ padding: '2rem', display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2rem', alignItems: 'start' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1.5rem' }}>
                            <div style={{ flex: 1, padding: '1.25rem', background: 'white', borderRadius: '1rem', border: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                                <div style={{ padding: '0.75rem', background: '#EEF2FF', borderRadius: '0.75rem' }}>
                                    <DollarSign size={22} color="#4F46E5" />
                                </div>
                                <div>
                                    <p style={{ fontSize: '0.7rem', fontWeight: '700', color: '#6B7280', textTransform: 'uppercase', marginBottom: '0.2rem' }}>Today's Revenue</p>
                                    <h3 style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827' }}>₱{todayTotal.toFixed(2)}</h3>
                                </div>
                            </div>
                            <div style={{ flex: 1, padding: '1.25rem', background: 'white', borderRadius: '1rem', border: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                                <div style={{ padding: '0.75rem', background: '#ECFDF5', borderRadius: '0.75rem' }}>
                                    <ShoppingBag size={22} color="#10B981" />
                                </div>
                                <div>
                                    <p style={{ fontSize: '0.7rem', fontWeight: '700', color: '#6B7280', textTransform: 'uppercase', marginBottom: '0.2rem' }}>Total Sales Today</p>
                                    <h3 style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827' }}>{recentSales.length}</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ background: 'white', borderRadius: '1.25rem', border: '1px solid #E5E7EB', padding: '1.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                                <h3 style={{ fontSize: '1.1rem', fontWeight: '800', color: '#111827' }}>Available Products</h3>
                                <div style={{ position: 'relative' }}>
                                    <Search style={{ position: 'absolute', top: '10px', left: '10px', color: '#9CA3AF' }} size={16} />
                                    <input type="text" placeholder="Search..." style={{ padding: '0.5rem 1rem 0.5rem 2.25rem', borderRadius: '0.75rem', border: '1px solid #E5E7EB', outline: 'none', fontSize: '0.85rem', width: '200px' }} />
                                </div>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '1rem' }}>
                                {products.map(product => (
                                    <div key={product._id} onClick={() => addToCart(product)}
                                        style={{ background: '#F9FAFB', padding: '1.25rem', borderRadius: '1rem', border: '1px solid #E5E7EB', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}>
                                        <div style={{ width: '52px', height: '52px', background: '#EEF2FF', borderRadius: '0.875rem', display: 'grid', placeItems: 'center', margin: '0 auto 0.75rem', overflow: 'hidden' }}>
                                        ...
                                        <h4 style={{ fontWeight: '700', fontSize: '0.875rem', color: '#111827', marginBottom: '0.25rem' }}>{product.name}</h4>
                        ...
                        <div style={{ background: 'white', borderRadius: '1.25rem', border: '1px solid #E5E7EB', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #F3F4F6' }}>
                                <h3 style={{ fontSize: '1.1rem', fontWeight: '800', color: '#111827' }}>Recent Transactions</h3>
                            </div>
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
                                    <tr>
                                        <th style={{ padding: '0.875rem 1.5rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: '700', color: '#6B7280' }}>RECEIPT</th>
                                        <th style={{ padding: '0.875rem 1.5rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: '700', color: '#6B7280' }}>CUSTOMER</th>
                                        <th style={{ padding: '0.875rem 1.5rem', textAlign: 'right', fontSize: '0.75rem', fontWeight: '700', color: '#6B7280' }}>AMOUNT</th>
                                        <th style={{ padding: '0.875rem 1.5rem', textAlign: 'right', fontSize: '0.75rem', fontWeight: '700', color: '#6B7280' }}>TIME</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {recentSales.slice(0, 5).map(sale => (
                                        <tr key={sale._id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                                            ...
                                            <td style={{ padding: '1rem 1.5rem', fontSize: '0.85rem', color: '#111827' }}>{sale.customerName}</td>
                                            <td style={{ padding: '1rem 1.5rem', fontWeight: '800', fontSize: '0.85rem', color: '#111827', textAlign: 'right' }}>₱{sale.totalAmount.toFixed(2)}</td>
                                            <td style={{ padding: '1rem 1.5rem', textAlign: 'right', fontSize: '0.8rem', color: '#6B7280' }}>{new Date(sale.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div style={{
                        ...
                        {/* Card Header */}
                        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #F3F4F6' }}>
                            <h3 style={{ fontSize: '1rem', fontWeight: '800', color: '#111827', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                                <ShoppingBag size={18} color="#4F46E5" /> Current Order
                            </h3>
                            <label style={{ fontSize: '0.7rem', fontWeight: '700', color: '#6B7280', display: 'block', marginBottom: '0.4rem', textTransform: 'uppercase' }}>Select Customer</label>
                            <select onChange={(e) => setSelectedCustomer(customers.find(c => c._id === e.target.value))}
                                style={{ width: '100%', padding: '0.6rem 0.75rem', borderRadius: '0.75rem', border: '1px solid #E5E7EB', outline: 'none', background: '#F9FAFB', fontWeight: '600', fontSize: '0.875rem' }}>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.5rem' }}>
                            {cart.length === 0 ? (
                                <div style={{ textAlign: 'center', padding: '2rem 0', color: '#D1D5DB' }}>
                                    <ShoppingBag size={36} style={{ margin: '0 auto 0.5rem' }} />
                                    <p style={{ fontSize: '0.85rem', color: '#9CA3AF' }}>Cart is empty</p>
                                </div>
                            ) : cart.map(item => (
                                <div key={item.cartKey} style={{ padding: '0.75rem 0', borderBottom: '1px solid #F3F4F6' }}>
                                    {/* Item name + price row */}
                                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.4rem' }}>
                                        <div style={{ flex: 1 }}>
                                            <h5 style={{ fontSize: '0.875rem', fontWeight: '700', color: '#111827' }}>{item.name}</h5>
                                            <p style={{ fontSize: '0.75rem', color: '#6B7280' }}>₱{item.pricePerUnit} each{item.payDeposit ? ` + ₱${item.containerDeposit} deposit` : ''}</p>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', background: '#F3F4F6', padding: '0.2rem 0.5rem', borderRadius: '0.5rem' }}>
                                            ...
                                            <span style={{ fontSize: '0.875rem', fontWeight: '800', minWidth: '18px', textAlign: 'center', color: '#111827' }}>{item.qty}</span>
                        ...
                        <div style={{ borderTop: '1px solid #F3F4F6', padding: '1.25rem 1.5rem', background: '#FAFAFA' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.1rem', fontWeight: '900', color: '#111827', paddingTop: '0.625rem', borderTop: '2px dashed #E5E7EB', marginTop: '0.25rem' }}>
