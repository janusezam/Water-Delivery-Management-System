            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <main style={{ padding: '2rem' }}>
                    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                        <div>
                            <h2 style={{ fontSize: '1.75rem', fontWeight: '800', color: '#111827', letterSpacing: '-0.025em' }}>Operations</h2>
                            <p style={{ color: '#6B7280', fontSize: '0.925rem' }}>Monitor your daily sales and dispatch status.</p>
                        </div>
                        <div style={{ display: 'flex', gap: '1rem' }}>
                            <button 
                                onClick={() => navigate('/customers')}
                                style={{ background: 'white', color: '#4F46E5', border: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1.25rem', borderRadius: '0.75rem', fontWeight: '700', cursor: 'pointer' }}>
                                <Plus size={20} />
                                Register Customer
                            </button>
                            <button 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
                        {[
                            ...
                            <div key={i} style={{ padding: '1.5rem', background: 'white', borderRadius: '1rem', border: '1px solid #E5E7EB' }}>
                                ...
                                <p style={{ fontSize: '1.5rem', fontWeight: '800', color: '#111827' }}>{stat.value}</p>
                            </div>
                        ))}
                    </div>

                    <div style={{ background: 'white', borderRadius: '1rem', border: '1px solid #E5E7EB', padding: '2rem' }}>
                        <h4 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1.5rem', color: '#111827' }}>Recent Orders</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {recentDeliveries.length === 0 ? (
                                <p style={{ textAlign: 'center', color: '#6B7280', padding: '1rem' }}>No recent orders found.</p>
                            ) : (
                                recentDeliveries.map((order, i) => (
                                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', borderRadius: '0.75rem', border: '1px solid #F3F4F6' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                            <div style={{ padding: '0.5rem', background: '#F3F4F6', borderRadius: '0.75rem' }}><Truck size={20} color="#6B7280" /></div>
                                            <div>
                                                <p style={{ fontSize: '0.9rem', fontWeight: '700', color: '#374151' }}>{order.customer?.name || order.customerName || 'Guest'}</p>
                                                <p style={{ fontSize: '0.8rem', color: '#9CA3AF' }}>{order.deliveryAddress || 'No address'} • {new Date(order.createdAt).toLocaleTimeString()}</p>
                                            </div>
                                        </div>
                                        <span style={{ 
                                            fontSize: '0.75rem', 
